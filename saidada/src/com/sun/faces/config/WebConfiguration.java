...
public class WebConfiguration {
	...
	private final Map<BooleanWebContextInitParameter, Boolean> booleanContextParameters = new EnumMap<BooleanWebContextInitParameter, Boolean>(
			BooleanWebContextInitParameter.class);

	private final Map<WebContextInitParameter, String> contextParameters = new EnumMap<WebContextInitParameter, String>(
			WebContextInitParameter.class);

	private final Map<WebContextInitParameter, Map<String, String>> facesConfigParameters = new EnumMap<WebContextInitParameter, Map<String, String>>(
			WebContextInitParameter.class);

	private final Map<WebEnvironmentEntry, String> envEntries = new EnumMap<WebEnvironmentEntry, String>(
			WebEnvironmentEntry.class);

	private final Map<WebContextInitParameter, String[]> cachedListParams;

	private final Set<String> setParams = new HashSet<String>();
	...
	/**
	 * Adds all com.sun.faces init parameter names to a list. This allows callers to determine if a parameter was
	 * explicitly set.
	 * 
	 * @param servletContext
	 *            the ServletContext of interest
	...
	private void processJndiEntries(String contextName) {
		...
		// if (LOGGER.isLoggable(loggingLevel)) {
		// LOGGER.log(loggingLevel, "jsf.config.webconfig.enventryinfo", new Object[] {
		// contextName, entryName, value });
		// }
		// } else {
		// if (LOGGER.isLoggable(loggingLevel)) {
		// LOGGER.log(loggingLevel, "jsf.config.webconfig.enventry.clientencrypt", contextName);
		// }
		// }
		// }
